...
export default function ReikiIntroSection() {
  const { t, img, config, cp } = useHayc();
  const intro = config.reikiPageConfig.intro;
            ...
            <div className="banner-content pt-5 text-center">
              <div className="d-flex align-items-center justify-content-center mb-4">
                <h1
                  ...
                  {...cp('reikiPageConfig.intro.headline')}
                >
                  {t(intro.headline)}
                </h1>
              </div>
              <p className="text-white" {...cp('reikiPageConfig.intro.subline')}>
                {t(intro.subline)}
              </p>
              <a
                href={intro.ctaHref}
                className="btn btn-primary btn-small text-uppercase mt-3 btn-pill"
                {...cp('reikiPageConfig.intro.ctaLabel')}
              >
                {t(intro.ctaLabel)}
              </a>
            </div>
