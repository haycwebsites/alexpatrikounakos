...
export default function DanceStorySection() {
  const { t, img, config, cp } = useHayc();
  const { containerId, block1, block2 } = config.dancePageConfig.story;
          ...
          <div className="col-lg-7 col-md-12">
            <div className="image-holder">
              <img
                ...
                {...cp('dancePageConfig.story.block1.image')}
              />
            </div>
          </div>
          <div className="col-lg-5 col-md-12 align-content-center" data-aos="fade-right">
            <div className="info-detail bg-primary-light h-100 h-100 p-4 align-content-center">
              <h3
                ...
                {...cp('dancePageConfig.story.block1.title')}
              >
                {t(block1.title)}
              </h3>
              <span {...cp('dancePageConfig.story.block1.body')}>
                {t(block1.body)}
              </span>
              ...
              <a
                ...
                href={block1.ctaHref}
                {...cp('dancePageConfig.story.block1.ctaLabel')}
              >
                {t(block1.ctaLabel)}
              </a>
            </div>
        ...
        <div className="row gy-4">
          ...
          >
            <div className="info-detail bg-primary-light h-100 h-100 p-4 align-content-center">
              <h3
                className="text-black mb-4"
                {...cp('dancePageConfig.story.block2.title')}
              >
                {t(block2.title)}
              </h3>
              <span {...cp('dancePageConfig.story.block2.body')}>
                {t(block2.body)}
              </span>
              ...
              <a
                className="btn btn-primary btn-small text-uppercase mt-3 btn-pill"
                href={block2.ctaHref}
                {...cp('dancePageConfig.story.block2.ctaLabel')}
              >
                {t(block2.ctaLabel)}
              </a>
            </div>
          ...
          <div className="col-lg-7 col-md-12 order-1 order-lg-2">
            <div className="image-holder">
              <img
                className="img-fluid wp-image-1283"
                src={img(block2.image)}
                alt={t(block2.imageAlt)}
                {...cp('dancePageConfig.story.block2.image')}
              />
            </div>
          </div>
        </div>
