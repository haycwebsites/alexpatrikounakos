...
export default function SelfAwarenessFaqSection() {
  const { t, config, cp } = useHayc();
  const faq = config.selfAwarenessPageConfig.faq;
  ...
  const renderItem = (item: (typeof faq.items)[0], idx: number) => {
    return (
      <FaqCollapse
        ...
        className="lc-block mb-5 border rounded p-3"
        headerClassName={(open) =>
          `fw-bold text-decoration-none text-dark lead${open ? '' : ' collapsed'}`
        }
        bodyClassName="py-4 text-dark"
        header={
          <span {...cp(`selfAwarenessPageConfig.faq.items.${idx}.question`)}>
            {t(item.question)}
          </span>
        }
      >
        <span {...cp(`selfAwarenessPageConfig.faq.items.${idx}.answer`)}>
          {t(item.answer)}
        </span>
      </FaqCollapse>
    );
        ...
        <div className="row mb-4">
          <div className="col-md-12">
            <div className="lc-block text-center">
              <h2
                className="fw-bold"
                {...cp('selfAwarenessPageConfig.faq.title')}
              >
                {t(faq.title)}
              </h2>
            </div>
            <div className="lc-block text-center">
              <p {...cp('selfAwarenessPageConfig.faq.subtitle')}>
                {t(faq.subtitle)}
              </p>
            </div>
          </div>
