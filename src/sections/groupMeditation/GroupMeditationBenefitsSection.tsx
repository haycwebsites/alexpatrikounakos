...
export default function GroupMeditationBenefitsSection() {
  const { t, config, cp } = useHayc();
  const { sectionTitle, cards } = config.groupMeditationPageConfig.benefits;
      ...
      <div className="container-md">
        <div className="text-center mb-4">
          <h2
            ...
            {...cp('groupMeditationPageConfig.benefits.sectionTitle')}
          >
            {t(sectionTitle)}
          </h2>
        </div>
          ...
          {cards.map((card, index) => (
            <div key={index} className="col-lg-4 col-md-6">
              <div className="border bg-white text-dark p-4 d-flex flex-column h-100 justify-content-center hoverBox">
                ...
                <p {...cp(`groupMeditationPageConfig.benefits.cards.${index}.body`)}>
                  {t(card.body)}
                </p>
              </div>
            </div>
