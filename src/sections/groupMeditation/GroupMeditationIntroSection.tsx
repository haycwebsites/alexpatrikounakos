...
export default function GroupMeditationIntroSection() {
  const { t, img, config, cp } = useHayc();
  const intro = config.groupMeditationPageConfig.intro;
            ...
            <div className="banner-content pt-5 text-center">
              <div className="d-flex align-items-center justify-content-center mb-4">
                <h1
                  ...
                  {...cp('groupMeditationPageConfig.intro.headline')}
                >
                  {t(intro.headline)}
                </h1>
              </div>
              <p className="text-white" {...cp('groupMeditationPageConfig.intro.subline')}>
                {t(intro.subline)}
              </p>
              <a
                href={intro.ctaHref}
                className="btn btn-primary btn-small text-uppercase mt-3 btn-pill"
                {...cp('groupMeditationPageConfig.intro.ctaLabel')}
              >
                {t(intro.ctaLabel)}
              </a>
            </div>
